          ...
          sidebarPanel(
              ...
              conditionalPanel(condition = "input.userinterface == 'Run Marxan'",
                  ...
                  selectInput("feature", "Choose a species to edit:",
                              choices = SelectInputFeatureChoices()),
                  numericInput("prop", "Target:",sProp,min=0,max=1,step=0.1),
                  numericInput("spf", "SPF:",sSPF,min=0),
                  br(),
    ...
    if (!is.null(input$database))
    {
        ...
        if (sPrevious != sMarxanDir)
        {
            if (sSelectDb != "")
            {
                ...
                if (exists("specnames"))
                {
                    updateSelectInput(session, "feature",choices = SelectInputFeatureChoices())
                }
                # update BLM
