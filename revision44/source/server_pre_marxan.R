...
JoinParallelResults_MarZone <- function(sMarxanDir,iCores,iRepsPerCore,iZones)
{
    ...
    # join ssoln files
    #cat(paste0("before join ssoln files\n"))
    ssolntable_ <- read.csv(paste0(sMarxanDir,"/output/output1_ssoln.csv"))
    #cat(paste0("before make selectionfreqs\n"))
    selectionfreqs <- rep(0,nrow(ssolntable_))
    for (i in 2:iZones)
    {
      selectionfreqs <- cbind(selectionfreqs,rep(0,nrow(ssolntable_)))
    }
    #cat(paste0("before ssoln table 1 iZones ",iZones," ncol(selectionfreqs) ",ncol(selectionfreqs)," ncol(ssolntable_) ",ncol(ssolntable_)," nrow(selectionfreqs) ",nrow(selectionfreqs)," nrow(ssolntable_) ",nrow(ssolntable_),"\n"))

    #save(iZones,selectionfreqs,ssolntable_,file="/mnt/shiny/users/matt/marzone/CockburnSound_Scenario3/output/test1.Rdata")
    ...
    #cat(paste0("before ssoln table 2 to 10\n"))
    for (i in 2:iCores)
    {
...
# filter list.dirs so that only dirs with /pulayer/leaflet.Rdata are included
#filter.list.dirs <- function(sSearchPath,path=".", pattern=NULL, all.dirs=FALSE,
#                             full.names=FALSE, ignore.case=FALSE)
#{
#  x <- list.dirs(sSearchPath,pattern=pattern,all.dirs=all.dirs,full.names=full.names,ignore.case=ignore.case)
#  
#  y <- c()
#  for (i in 1:length(x))
#  {
#    #i <- 1
#    #i <- 2
#    z <- paste0(sAppHome,"/",x[i],"/pulayer/leaflet.Rdata")
#    if (file.exists(z))
#    {
#      y <- c(y,x[i])
#    }
...
map_mz_ssolnNmap_leaflet <- function(iN)
{
    ...
    save(leaflet_colours,leaflet_proj10,file=paste0(sMarxanDir,"/pulayer/ssolnNmap.Rdata"))
}
...
map_mz_runMmap_leaflet <- function(iM)
{
    ...
    cat(paste0("length(display_polygon) ",length(display_polygon),"\n"))

    cat(paste0("class(proj10) ",class(proj10),"\n"))
    ...
    cat(paste0("class(leaflet_proj10) ",class(leaflet_proj10),"\n"))

    cat(paste0("dim(leaflet_proj10@data) ",dim(leaflet_proj10@data),"\n"))
    ...
    cat(paste0("length(leaflet_colours) ",length(leaflet_colours),"\n"))
    cat(paste0("head(leaflet_colours) ",head(leaflet_colours),"\n"))

    save(leaflet_colours,leaflet_proj10,display_polygon,proj10,file=paste0(sMarxanDir,"/pulayer/runMmap.Rdata"))
}
...
map_ssolnNmap_leaflet <- function()
{
    ...
    if (!fMask)
    {
        leaflet_colours <<- colours
        leaflet_proj10 <<- proj10
    } else {
        cat(paste0("class(proj10) ",class(proj10),"\n"))
        ...
        cat(paste0("class(PUID) ",class(PUID),"\n"))
...
binary_map_leaflet <- function(values)
{
    ...
    save(leaflet_colours,leaflet_proj10,file=paste0(sMarxanDir,"/pulayer/binarymap.Rdata"))
}
