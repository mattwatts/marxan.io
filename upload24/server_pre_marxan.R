...
AuthenticateUserSession <- function(sessionkey)
{
    ...
    if (file.exists(sSessionKeyFile))
    {
        ...
        if (TRUE) #(sUserIP == sSessionUserIP)
        {
...
AuthenticateUser_login <- function()
{
    ...
    sLogFile <<- CreateLogFile(paste0(sShinyUserPath,sUserName),sUserName,sUploadApp)
...
ChangeDatabase <- function()
{
    ...
    if (length(list.files(path=paste0(sMarxanDir,"/output"))) == 0)
    {
        ...
        RunMarxan_paramtest_1st(sMarxanDir,sShinyDataPath,iCores,iParamTestReps)
    }
    SelectDatabase()
    PrepareDisplay()
